...
class SstsourcePipeline:

    # Database setup
    conn = sqlite3.connect('stsource.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `stsource`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM stsource WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO stsource (date, title, link, content)"
